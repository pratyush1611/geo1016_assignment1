        ...
        vec3& t)
{
...
#if 1
    /**
     * [Liangliang:] Camera calibration requires computing the SVD and inverse of matrices. The matrix classes (i.e.,
     * mat3 and mat4) you've implemented in the first assignment only support matrices of a fixed-dimension. However,
     * the input to the calibration problem can be arbitrary size (i.e., n>=6). Thus, in this assignment, I provide you
     * with such an implementation (see matrix.h). I also wrote the following code just to show you how to
     *   - use the dynamic 1D array data structure 'std::vector' from the standard C++ library; The points (both 3D and
     *     2D are) stored in such arrays;
     *   - use the template matrix class (which can have arbitrary size);
     *   - compute the SVD of a matrix;
     *   - compute the inverse of a matrix;
     *   - compute the transpose of a matrix.
     *   TODO: you should delete the following code in you final submission.
     */
    ...
    for (int i=0; i<array.size(); ++i)
        std::cout << array[i] << " ";  // use 'array[i]' to access it i-th element.
    std::cout << std::endl;
